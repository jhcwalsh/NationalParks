...
# ── Facility seed data ───────────────────────────────────────────────────────
# Loaded from data/facilities.json (generated by fetch_facilities.py).
# Falls back to a small hardcoded set if the JSON doesn't exist yet.

_FACILITIES_JSON = _PROJECT_ROOT / "data" / "facilities.json"

_FALLBACK_FACILITIES: list[dict[str, Any]] = [
...
def _load_facilities() -> list[dict[str, Any]]:
    """Load facilities from JSON file, falling back to hardcoded list."""
    if _FACILITIES_JSON.exists():
        try:
...
async def init_db() -> None:
    ...
    async with aiosqlite.connect(_db_path()) as conn:
        ...
        # Seed facilities from JSON (or fallback)
